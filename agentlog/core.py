...
TOOL_NAME = "AGENTLOG"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'AL-MISS-001', 'severity': 'high', 'weight': 2.5, 'title': 'MISSING_TRACE_ID', 'when': {'trace_id': '^$'}, 'remediation': 'Emit OTel trace_id on every step.'}, {'id': 'AL-SEC-001', 'severity': 'critical', 'weight': 3.0, 'title': 'TOOL_CALL_NO_AUTH', 'when': {'auth_checked': False, 'action': 'tool_call'}, 'remediation': 'Verify auth before tool execution.'}, {'id': 'AL-PII-001', 'severity': 'high', 'weight': 2.5, 'title': 'PII_IN_TRACE', 'when': {'contains_pii': True}, 'remediation': 'Redact PII before logging.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="agent-audit",
                    metadata=item,
                ))
